...
def solution(game_board, table):
    ...
    puzzle_list = puzzle_to_puzzle_list(puzzle_blanks[2])
    print_board(puzzle_list)
    print_board(puzzle_list_turn_right_90(puzzle_list))
...
def puzzle_rotation_for_angle(puzzle):
    puzzles = [puzzle]
    for i in range(3):
        puzzle.append(puzzle_list_turn_right_90(puzzle))
    return puzzles
...
def puzzle_list_turn_right_90(puzzle_list):
    ...
    for y in range(original_y_len):
        for x in range(original_x_len):
            # print(x, y, ' -> ', y, x*-1)
            turn_puzzle_list[original_x_len - x - 1][y] = puzzle_list[y][x]
